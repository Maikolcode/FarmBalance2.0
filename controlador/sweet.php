<html>
    <head>
        <meta charset="UTF-8">
        <title>Farm Balance</title>
        <link rel="stylesheet" href="../multimedia/style.css"/> 
        <link rel="icon" href="../multimedia/img/primerLogo.svg" type="image/png"/>
        <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/materialize/1.0.0/css/materialize.min.css">
        <script src="../multimedia/sweetalert2/sweetalert2.all.min.js"></script>
    </head>
    <body style="background-color: rgba(255, 255, 46, 0.5)">
        <a href="../index.php"><img src="../multimedia/img/segundoLogo.svg" style="height: 100%; width: 100%"/></a>
    </body>
</html>